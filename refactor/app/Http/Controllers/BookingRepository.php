...
class BookingRepository extends BaseRepository
{
     ...
     * @return array
     */
    public function getUsersJobs(int $userID)
    {
    ...
    public function getUsersJobsHistory(int $userID, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $jobIds = Job::getTranslatorJobsHistoric($cuser->id, 'historic', $pagenum);
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function jobToData($job)
    {
        ...
        $data['job_for'] = array();
        if ($job->gender != null && in_array($job->gender, ['male', 'female'])) {
    ...
    public function getPotentialJobIdsWithUserId($userID)
    {
        ...
        $job_ids        = Job::getJobs($userID, $job_type, 'pending', $userlanguage, $gender, $translatorLevel);
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function endJob($post_data)
    {
        ...
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        $sessionExplode = explode(':', $job->session_time);
        $session_time = $sessionExplode[0] . ' tim ' . $sessionExplode[1] . ' min';
        $data = [
            'user'         => $user,
            'job'          => $job,
            'session_time' => $session_time,
